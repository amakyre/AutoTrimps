...
function autoStance() {
    ...
    if (!game.global.preMapsActive && game.global.soldierHealth > 0) {
        ...
        } else if (game.global.world == 200 || (game.upgrades.Dominance.done && ((newSquadRdy && dHealth > dDamage) || dHealth - missingHealth > dDamage) && doubleAttackOK && leadAttackOK && voidCritinDok)) {
            setFormation(2);
...
//        (game.global.world >= 210 && game.global.lastClearedCell > 93 && ((new Date().getTime() - game.global.mapStarted > 8000 && game.global.mapsActive) || game.global.mapBonus <= 9 )) ||
    //    (game.global.world == 230 && game.global.lastClearedCell > 79 && ((new Date().getTime() - game.global.zoneStarted) / 1000 / 60) < 3) ||
        (game.global.world >= 216 && game.global.world <= 220 && game.global.mapBonus < 1)) {  //didnt work (game.global.mapBonus < game.global.world-(game.upgrades.Coordinated.level+1)
...
function manageGenes() {
    ...
    else if ((targetBreed < getBreedTime(true) || (game.resources.trimps.soldiers == 0 && getBreedTime(true) > 6)) && breedFire == false && (getPageSetting('BreedFire') || game.global.world == 200) && game.global.world > 10) {
        breedFire = true;
    }
    /*    
